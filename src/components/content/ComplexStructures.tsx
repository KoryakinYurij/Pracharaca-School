...
export function StepTimeline({ steps, className }: StepTimelineProps) {
  return (
    <ol className={clsx('relative ml-4 space-y-12 border-l-2 border-border/50 pl-8', className)}>
      {steps.map((step, index) => (
        <li key={step.title || `step-${index}`} className="relative group">
          <div aria-hidden="true" className="absolute -left-[2.25rem] top-0 h-4 w-4 rounded-full border-2 border-gold bg-ivory transition-colors duration-300 group-hover:bg-gold" />
