...
export function PrevNextNav({ prev, next, topicSlug }: PrevNextNavProps) {
  return (
    <nav className="mt-16 grid gap-3 border-t border-border/80 pt-6 sm:grid-cols-2 sm:gap-4" aria-label={t('nav.ariaLabel')}>
      {prev ? (
        <Link
        ...
        >
          ...
          <span className="min-w-0">
            <span className="block text-xs uppercase tracking-[0.12em] text-graphite/70">{t('nav.previous')}</span>
      ...
      {next ? (
        ...
        >
          <span className="min-w-0">
            <span className="block text-xs uppercase tracking-[0.12em] text-graphite/70">{t('nav.next')}</span>
